...
public class ChessGUI extends JFrame {
    ...
    private void updateBoardHighlights() {
        ...
        if (lastMoveFrom != null) {
            chessBoard.setSquareHighlight(lastMoveFrom.getRow(), lastMoveFrom.getColumn(),
                    ChessBoard.HighlightType.LAST_MOVE);
        }
        if (lastMoveTo != null) {
        ...
        if (selectedSquare != null) {
            chessBoard.setSquareHighlight(selectedSquare.getRow(), selectedSquare.getColumn(),
                    ChessBoard.HighlightType.SELECTED);

            // Movimentos legais
            for (Position move : legalMoves) {
                chessBoard.setSquareHighlight(move.getRow(), move.getColumn(),
                        ChessBoard.HighlightType.LEGAL_MOVE);
            }
        }
            ...
            for (int row = 0; row < 8; row++) {
                for (int col = 0; col < 8; col++) {
                    ...
                    if (piece != null && piece.getSymbol().equals("K") &&
                        ...
                        chessBoard.setSquareHighlight(row, col, ChessBoard.HighlightType.CHECK);
                        break;
                    }
                }
            }
    ...
    private JPanel createButtonsGrid() {
        ...
        Button newGameButton = new Button("Novo Jogo", Button.ButtonStyle.SUCCESS);
        newGameButton.addActionListener(e -> startNewGame());
        buttonsPanel.add(newGameButton);

        Button surrenderButton = new Button("Desistir", Button.ButtonStyle.DANGER);
        surrenderButton.addActionListener(e -> surrenderGame());
        buttonsPanel.add(surrenderButton);

        // Linha 2: Salvar e Carregar
        Button saveButton = new Button("Salvar", Button.ButtonStyle.PRIMARY);
        saveButton.addActionListener(e -> saveGame());
        buttonsPanel.add(saveButton);

        Button loadButton = new Button("Carregar", Button.ButtonStyle.WARNING);
