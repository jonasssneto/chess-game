 ...
 *
 * Mantém um cache LRU por (filename|size) de ImageIcon escalado com alta
 * qualidade.
 */
public final class ImageUtil {
    ...
    private static final Map<String, ImageIcon> ICON_CACHE = new LinkedHashMap<>(128, 0.75f, true) {
        @Override
    ...
    public static ImageIcon getPieceIcon(boolean isWhite, char pieceChar, int size) {
        ...
        // Debug: sempre mostra o que está tentando carregar
    ...
    /**
     * Carrega um ImageIcon do resources, redimensionando para size x size com alta
     * qualidade.
    ...
    public static BufferedImage loadBuffered(String filename) {
        ...
        // 1) Classpath raiz (para quando executamos com -cp out:resources)
        try {
            ...
            System.out.println("  -> ERRO ao ler de classpath raiz: " + e.getMessage());
        }

        // 2) Classpath com prefixo /resources/
        try {
            URL url = ImageUtil.class.getResource(CLASSPATH_PREFIX + filename);
            if (url != null) {
                System.out.println("  -> ENCONTROU em classpath prefixo: " + url);
                return ImageIO.read(url);
            } else {
                System.out.println("  -> NÃO encontrou em classpath prefixo: " + CLASSPATH_PREFIX + filename);
            }
        } catch (Exception e) {
            System.out.println("  -> ERRO ao ler de classpath prefixo: " + e.getMessage());
        }
        ...
        try {
            ...
            if (f.exists()) {
                System.out.println("  -> ENCONTROU arquivo local: " + f.getAbsolutePath());
                return ImageIO.read(f);
            } else {
                System.out.println("  -> NÃO encontrou arquivo local: " + f.getAbsolutePath());
            }
        } catch (Exception e) {
            System.out.println("  -> ERRO ao ler arquivo local: " + e.getMessage());
        }

        System.out.println("  -> FALHA TOTAL ao carregar: " + filename);
    ...
    /**
     * Escala com Graphics2D e hints de alta qualidade (melhor que
     * getScaledInstance).
     */
